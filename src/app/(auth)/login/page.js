...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      logger.info('Initiating Google sign in', { callbackUrl });
      ...
      if (result?.error) {
        ...
        logger.info('Google sign in successful, redirecting', { url: result.url });
    ...
    <div className="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-gray-50">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          Discover and bookmark design hackathons
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <Card className="py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          {/* Alternative login form (for future implementation) */}
          <div className="mt-6">
            <LoginForm 
              onSuccess={(data) => {
                logger.info('Login form success', { email: data.email });
                router.push(callbackUrl);
              }}
              onError={(error) => {
